...
interface DashboardPageProps {
  ...
  invoices?: Invoice[];
  avocats?: Avocat[];
  onUpdateTaskStatus?: (id: string, status: 'Effectué' | 'Non effectué' | 'Effectué à moitié') => void;
                                    ...
                                    <div key={lawyer.id} className={`p-4 rounded-xl border transition ${hasOverdue ? 'bg-rose-50/10 border-rose-100' : 'bg-slate-50/40 border-slate-100 hover:border-gray-200'}`}>
                                        <div className="flex items-start justify-between gap-4">
                                            ...
                                            <div className="flex flex-col items-end gap-1 text-right">
                                                {hasOverdue ? (
                                                    <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[9px] bg-rose-100 border border-rose-250 text-rose-800 font-black uppercase">
                                                        ⚠️ {lawyer.overdue} en retard
                                                    </span>
                                                ) : (
                                                    <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[9px] bg-green-100 border border-green-250 text-green-800 font-bold uppercase">
                                                        ✓ En ordre
                                                    </span>
                                                )}
                                ...
                                {upcomingEvents.slice(0, 5).map(h => (
                                    <li key={h.id} className="p-3 bg-slate-50 hover:bg-indigo-50/20 rounded-xl border border-slate-100 transition duration-150">
                                         ...
                                         <div className="flex items-center justify-between mt-2">
                                             <span className="text-xs text-gray-400 font-medium flex items-center gap-1">📍 {h.lieu}</span>
                                         ...
                                         <p className="text-2xs font-extrabold text-[#15447c] mt-1.5 uppercase tracking-wide">
                                             📅 {h.date}
                                         </p>
                                    </li>
                                ))}
                            ...
                            {!quickAddOpen && (
                                <button 
                                    onClick={() => {
                                        ...
                                        if (cases.length > 0 && !qCaseId) {
                                            setQCaseId(cases[0].id);
                                        }
                                    }}
                        ...
                        <form onSubmit={handleQuickAddSubmit} className="flex-1 flex flex-col justify-between overflow-y-auto custom-scrollbar pr-1 bg-slate-50/50 p-4 rounded-xl border border-slate-100 animate-fadeIn">
                            <div className="space-y-2.5">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    <div>
                                        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-1">Dossier lié</label>
                                        <select 
                                            value={qCaseId}
                                            ...
                                            <div className="flex items-start justify-between">
                                                <div className="flex items-start gap-2.5 min-w-0 flex-1">
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        ...
                                                        <p className="text-3xs text-gray-400 mt-0.5 font-medium truncate">📜 Dossier : {t.caseId}</p>
                                                    </div>
                                                </div>
