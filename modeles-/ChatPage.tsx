...
const ChatPage: FC<ChatPageProps> = ({ avocats = [], personnels = [] }) => {
    const contacts = [
        ...avocats.map((a) => ({ name: a.fullName, role: a.cabinetRole, status: a.serviceStatus === 'Actif' ? 'online' as const : 'offline' as const })),
        ...personnels.map((p) => ({ name: p.fullName, role: p.role, status: p.serviceStatus === 'Actif' ? 'online' as const : 'offline' as const })),
    ];
    // Nav Tabs
    ...
    const filteredPersonnel = contacts.filter(p =>
                ...
                <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 w-fit">
                    ...
                    >
                        ...
                        <span className="bg-green-500 text-white font-extrabold text-[10px] px-1.5 py-0.2 rounded-full">
                            {contacts.filter(p => p.status === 'online').length}
                        </span>
                    </button>
                    <button
                        type="button"
