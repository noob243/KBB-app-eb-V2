...
const Sidebar: FC<SidebarProps> = ({ currentPage, setCurrentPage, onLogout }) => {
    const navItems = [
        ...
        { name: 'AIAssistant', icon: <AIIcon />, label: "Otsudi AI" },
        { name: 'Clients', icon: <ClientsIcon />, label: "Clients" },
        { name: 'Dossiers', icon: <CasesIcon />, label: "Dossiers" },
        { name: 'Evenements', icon: <EventsIcon />, label: "Événements" },
    ...
    return (
        <aside className="bg-slate-900 text-slate-100 w-64 space-y-2 p-4 flex flex-col shadow-xl z-20">
            <div className="text-2xl font-bold font-outfit mb-6 flex items-center p-2 text-white">
            ...
            <nav className="flex-1 overflow-y-auto custom-scrollbar">
                {navItems.map(item => (
                    <a
                        ...
                        className={`flex items-center px-4 py-3 mb-1 rounded-xl text-sm font-medium transition-all duration-200 ${currentPage === item.name ? 'bg-indigo-600 text-white shadow-md shadow-indigo-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
                    >
                        <Icon>{item.icon}</Icon>
            ...
            <div className="mt-auto pt-4 border-t border-slate-800">
                 <a href="#" onClick={(e) => { e.preventDefault(); onLogout(); }} className="flex items-center px-4 py-3 rounded-xl text-sm font-medium text-slate-400 hover:bg-slate-800 hover:text-white transition-all duration-200">
