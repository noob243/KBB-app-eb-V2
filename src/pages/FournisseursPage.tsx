...
const FournisseursPage: FC<FournisseursPageProps> = ({ fournisseurs, onAddFournisseur, onDeleteFournisseur }) => {
    ...
    const getNatureClass = (nature: 'Bien' | 'Services') => {
                                                ...
                                                >
                                                    ...
                                                    <td className="p-4">
                                                        <div>
                                                            ...
                                                            <p className="text-[10px] text-gray-400 font-bold mt-0.5">👤 {f.dirigeantPrincipal || 'Non spécifié'}</p>
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td className="p-4 text-right" onClick={(e) => e.stopPropagation()}>
                                                        <div className="flex items-center justify-end gap-1.5">
                                                            <button 
                                                                onClick={() => setSelectedFournisseur(f)}
                                                                className="px-2.5 py-1 text-2xs font-extrabold text-indigo-700 bg-indigo-50 hover:bg-indigo-100/80 rounded-lg border border-indigo-100 transition"
                                                            >
                                                                Voir
                                                            </button>
                                                            <button 
                        ...
                        {selectedFournisseur ? (
                            <div className="space-y-6">
                                <div className="border-b border-gray-150 pb-4">
                                    ...
                                    <h3 className="text-base font-black text-gray-800 mt-2.5 leading-snug">{selectedFournisseur.nomComplet}</h3>
                                    <p className="text-xs text-gray-500 mt-0.5 flex items-center gap-1.5 font-semibold">
                                        <span>📧</span>
                                        {selectedFournisseur.adresseMail ? (
                                            <a href={`mailto:${selectedFournisseur.adresseMail}`} className="text-indigo-650 hover:text-indigo-800 hover:underline transition">
                                                {selectedFournisseur.adresseMail}
                                            </a>
                                        ) : (
                                            <span className="text-gray-400">Aucune adresse mail</span>
                                        )}
                                    </p>
                                </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-2 text-xs font-semibold">
                                        <div className="bg-slate-50/50 p-2.5 rounded-xl border border-slate-100 flex justify-between items-center">
                                            <span className="text-gray-400">👤 Dirigeant Principal :</span>
                                        ...
                                        <div className="bg-slate-50/50 p-2.5 rounded-xl border border-slate-100 flex justify-between items-center">
                                            <span className="text-gray-400">🏛️ Nature de Preuve :</span>
                                    ...
                                    <div className="border-t border-gray-150 pt-4">
                                        <h4 className="text-2xs font-extrabold text-indigo-700 uppercase tracking-widest mb-3 flex justify-between items-center">
                                            <span>📞 Référents chez le prestataire</span>
                                                ...
                                                {selectedFournisseur.referents.map((ref, i) => (
                                                    <div key={i} className="p-3 bg-slate-50 border border-slate-100 rounded-xl space-y-1">
                                                        <p className="text-xs font-black text-gray-800 flex items-center gap-1.5">
                                                            <span className="text-3xs">👤</span> {ref.nom}
                                                        </p>
                                                        {ref.phone && (
